        ...
        for measure in xml_tree_measure:
            ...
            for idx, note in enumerate(xml_tree_measure_notes):
                ...
                if(note_duration != None) :
                    if(int(note_duration.text) >  (3*9)):
                        xml_tree_duration_value = str(int(note_duration.text)-(3*9)) + " I9 I9 I9"
                    elif(int(note_duration.text) >  (2*9)):
                        xml_tree_duration_value = str(int(note_duration.text)-(2*9)) + " I9 I9"
                    elif(int(note_duration.text) >  (9)):
                        xml_tree_duration_value = str(int(note_duration.text)-9) + " I9"
                    else : 
                        xml_tree_duration_value = note_duration.text;
                    print("["+str(idx)+"]"+'duration '+ str(note_duration.text)+ ' ')
                ...
                if(xml_tree_pitch != None) :
                    ...
                    #result_lyre_song += str(xml_tree_pitch_step_value)+str(note_duration.text)+" "
                ...
                #     result_lyre_song += "q"+str(note_duration.text)+" "
                # elif(xml_tree_step.text == 'F'):
                #     result_lyre_song += "w"+str(note_duration.text)+" "
                # elif(xml_tree_step.text == 'G'):
                #     result_lyre_song += "e"+str(note_duration.text)+" "
                # elif(xml_tree_step.text == 'A'):
                #     result_lyre_song += "r"+str(note_duration.text)+" "
                # elif(xml_tree_step.text == 'B'):
                #     result_lyre_song += "t"+str(note_duration.text)+" "
                # elif(xml_tree_step.text == 'C'):
                #     result_lyre_song += "y"+str(note_duration.text)+" "
                # elif(xml_tree_step.text == 'D'):
                #     result_lyre_song += "u"+str(note_duration.text)+" "
                # else:
                #     result_lyre_song += ""
                    ...
                    for idx, attribute in enumerate(xml_tree_measure_attributes):
                        ...
                        if(xml_tree_measure_attributes_clef != None) :
                            ...
                            if(xml_tree_measure_attributes_clef_sign != None) :
                                ...
                                sheet_settings += "_"+str(xml_tree_measure_attributes_clef_sign.text)
                ...
                if(xml_tree_measure_notes != []) :
                    ...
                    for idx, note in enumerate(xml_tree_measure_notes):
                        ...
                        if(xml_tree_rest != None) :
                            ...
                            result_note_song += "I"+str(note_duration.text)+" "
                        #define note pitch
                        xml_tree_pitch = note.find('pitch')
                        if(xml_tree_pitch != None) :
                            xml_tree_pitch_step = xml_tree_pitch.find('step')
                            ...
                            if((xml_tree_pitch_octave != None) & (xml_tree_pitch_step != None)) :
                                ...
                                result_note_song += str(xml_tree_pitch_step_value)+str(note_duration.text)+" "
                        #define note type
                        xml_tree_type = note.find('type')
                        if(xml_tree_type != None) :
